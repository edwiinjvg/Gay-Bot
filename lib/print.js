...
export default async function (m, conn = { user: {} }) {
    ...
    console.log(`${chalk.black(chalk.bgCyanBright('%s'))} ${chalk.whiteBright(oraColombia)}

📍 : ${chalk.magentaBright('%s')}
🎬 : ${chalk.greenBright('%s')} ${chalk.gray('[%s %sB]')}
🙎 : ${chalk.yellow('%s')}
📆 : ${chalk.blue('%s%s')}
👤 : ${chalk.redBright(chatName)}
🔥 : ${chalk.whiteBright('%s')}
`.trim(),
